...
GRID_SIZE = 50
OBSTACLE_DENSITY = 0.3  # 20% prekážok
NUM_RUNS = 10
...
def euclidean_heuristic(a, b):
    """Euklidovská vzdialenosť (pre 8-smerový pohyb)"""
    return math.sqrt((a[0] - b[0])**2 + (a[1] - b[1])**2)

def a_star(grid, start, goal, heuristic):
    """A* algoritmus s danou heuristikou"""
    size = len(grid)
...
def main():
    ...
    for run in range(1, NUM_RUNS + 1):
        ...
        if result['success']:
            ...
            print(f"  - Dĺžka cesty: {result['path_length']:.2f}")
    ...
    if successful_runs > 0:
        ...
        print(f"  Dĺžka cesty: {total_path / successful_runs:.2f}")
